...
# --- ส่วนที่ 2: Visualizations ---
    st.write("### 📈 แนวโน้มและตารางข้อมูล")
    # เพิ่ม "📋 ตารางข้อมูล" เข้าไปใน tabs
    tab1, tab2, tab3 = st.tabs(["📊 แนวโน้มการจัดการ", "🌦️ ปัจจัยสภาพอากาศ", "📋 ตารางข้อมูล"])
    
    with tab1:
        st.write("เปรียบเทียบปริมาณขยะจริง กับความสามารถในการรองรับของถังขยะ")
    ...
    with tab3:
        st.write("ข้อมูลทั้งหมดที่ผ่านการกรองแล้ว (คุณสามารถคลิกที่หัวตารางเพื่อเรียงข้อมูลได้)")
        # ใช้ use_container_width เพื่อให้ตารางขยายเต็มหน้าจอ
        st.dataframe(filtered_df, use_container_width=True)
        
        # เพิ่มปุ่มดาวน์โหลดเฉพาะข้อมูลที่เลือกในตารางนี้
        csv_data = filtered_df.to_csv(index=False).encode('utf-8')
        st.download_button(
            label="📥 ดาวน์โหลดข้อมูลในตารางนี้เป็น CSV",
            data=csv_data,
